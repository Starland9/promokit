...
def cmd_init(a):
    if a.kind in ("learning", "learning-tiktok"): return cmd_concept(argparse.Namespace(name=a.name, num=None, title=a.title or a.name, angle=a.angle or "", series=a.series, handle=a.handle))
...
def cmd_concept(a):
    ...
    if num is not None:
        ...
        if row: title = title or row[0]; angle = angle or row[1]
        elif not title: sys.exit(f"concept #{num} not found in {KIT / '100-concepts-backend-tiktok.md'}; pass --title")
    if not title: sys.exit("give --num <n> (row of 100-concepts-backend-tiktok.md) and/or --title \"...\"")
    try: hk.scaffold(KIT, a.name, title, angle, num, a.series, a.handle)
    except (FileExistsError, FileNotFoundError) as e: sys.exit(str(e))

def cmd_hooks(a):
    slots = {"concept": a.concept} if a.concept else {}
    for kv in a.slot or []:
        k, _, v = kv.partition("="); slots[k.strip()] = v.strip()
    hk.print_hooks(slots, a.family)

def cmd_script(a):
    p = _project(a.project); pricing = Pricing(p.get("pricing")); n = hk.lint(p, pricing)
    if n and a.strict: sys.exit(1)

def cmd_words(a):
    p = _project(a.project); hk.word_report(p, only=a.only)
...
def main(argv=None):
    ...
    s = sub.add_parser("init", help="create a project from a template"); s.add_argument("name"); s.add_argument("--url"); s.add_argument("--kind", default="project", help="project (16:9 promo) | learning (TikTok concept video)"); s.add_argument("--title"); s.add_argument("--angle"); s.add_argument("--series", default="100 Concepts Backend"); s.add_argument("--handle", default="@starland9"); s.set_defaults(fn=cmd_init)
    s = sub.add_parser("concept", help="scaffold a learning TikTok project (free): --num 4 reads 100-concepts-backend-tiktok.md, or --title/--angle"); s.add_argument("name"); s.add_argument("--num", type=int); s.add_argument("--title"); s.add_argument("--angle"); s.add_argument("--series", default="100 Concepts Backend"); s.add_argument("--handle", default="@starland9"); s.set_defaults(fn=cmd_concept)
    s = sub.add_parser("hooks", help="hook formulas for educational videos (free)"); s.add_argument("--concept"); s.add_argument("--family"); s.add_argument("--slot", nargs="*", help="key=value, e.g. symptome=\"ta requête met 8 s\""); s.set_defaults(fn=cmd_hooks)
    s = sub.add_parser("script", help="lint the voice-over script: durations, hook, display, recap, cost (free)"); s.add_argument("project"); s.add_argument("--strict", action="store_true", help="exit 1 on warnings"); s.set_defaults(fn=cmd_script)
    s = sub.add_parser("words", help="word timings of the generated voice-over, for @mot references (free)"); s.add_argument("project"); s.add_argument("--only", nargs="*"); s.set_defaults(fn=cmd_words)
